...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    case 'n':
      ...
      break;
    case 'p': {
      supforms=UtfConverter::fromUtf8(optarg);
      break;
    }
    case 'd':
      Utils::debug=true;
  ...
  if (mode==MODE_TRAIN) {
    if (prune_m>0) {
      ...
      if (!fprob) tagger_utils::file_name_error(initprob);
	...
	exit(EXIT_FAILURE);
      ...
      if (!fdic) tagger_utils::file_name_error(filename+".dic");
      hmm_trainer.read_dictionary(fdic);
      fclose(fdic);
    }

    fcrp = fopen((filename+".crp").c_str(), "r");
    if (!fcrp) tagger_utils::file_name_error(filename+".crp");

    fprob = fopen((filename+".prob").c_str(), "w");
    if (!fprob) tagger_utils::file_name_error(filename+".prob");

    if (pathsfile!="") {
      fpaths.open(pathsfile.c_str(), ios::out | ios::trunc);
      if(fpaths.fail()) tagger_utils::file_name_error(pathsfile);
    }

    if(transfile!="") {
      ftrans.open(transfile.c_str(), ios::in);
      if (ftrans.fail()) tagger_utils::file_name_error(transfile);
    }

    if(likefile!="") {
      flike.open(likefile.c_str(), ios::in);
      if(flike.fail()) tagger_utils::file_name_error(likefile);
    }
  }
