...
int main(int argc, char* argv[]) {
  ...
  if(fcounts.fail()) tagger_utils::file_name_error(countsfile);

  ftagclus.open(tagclustersfile.c_str(), ios::in);
  if(ftagclus.fail()) tagger_utils::file_name_error(tagclustersfile);
  ...
  if (evaltagger) {
    ...
    if (!ftagged) tagger_utils::file_name_error(taggedfile);

    funtagged = fopen(untaggedfile.c_str(), "r");
    if (!funtagged) tagger_utils::file_name_error(untaggedfile);
