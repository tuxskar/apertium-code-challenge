...
HMM_TL_driven_trainer::train(FILE *is, int corpus_length, int save_after_nwords, string filename, ofstream& fpaths, ifstream& ftrans, ifstream& flike, bool savecounts){
  ...
  last_etq_segmento_ant=tagger_data.getTagIndex()[L"TAG_SENT"];
  ...
  while(true) {
    ...
    for(size_t npalabras=0; npalabras<(seg->vwords.size()); npalabras++) {
    ...
    for(size_t npalabras=0; npalabras<seg->vwords_before.size(); npalabras++){
      Utils::print_debug("   ");
    ...
    for(size_t npalabras=0; npalabras<seg->vwords_after.size(); npalabras++){
      Utils::print_debug("   ");
	...
	if (seg->get_number_paths()==1) {
	  ...
	  Utils::print_debug(" TO TRANSLATE: ");
	  ...
	  Utils::print_debug("TRANSLATION: ");
	  Utils::print_debug(tradcadena);
	  Utils::print_debug("\n");
	  if (tagger_utils::trim(tradcadena).length()==0) {
	    cerr<<"Error: There is a path with a null translation: "<<UtfConverter::toUtf8(cadena)<<"\n";
	    cerr<<"SEGMENT: ";
	    for(size_t npalabras=0; npalabras<(seg->vwords.size()); npalabras++)
	      cerr<<UtfConverter::toUtf8(seg->vwords[npalabras].get_superficial_form())<<" ";
	    cerr<<"\n";
    ...
    update_counts(seg, translations, tags_pair, emis, last_etq_segmento_ant, tags_count, ambclass_count, tags_count_for_emis);
	...
	cout<<(*itmap).first<<" "<<((double)trads_long_segmentos[TL1][(*itmap).first])/((double)(*itmap).second)<<"\n";
      ...
      SmoothUtils::calculate_smoothed_parameters(tagger_data, tags_count, tags_pair, ambclass_count, emis, tags_count_for_emis, cuenta_palabras);
...
HMM_TL_driven_trainer::train_pruning(FILE *is, int corpus_length, int save_after_nwords, string filename, double mixing_c, ifstream& ftrans, ifstream& flike, bool savecounts) {
  ...
  while(true) {
    ...
    map<int, wstring> paths_translations;
    map<wstring, double> translations_likelihoods;
    if (ftrans.is_open()) {
...
HMM_TL_driven_trainer::update_counts(Segment* seg, vector<Translations*> &trans, 
  ...
  for (int i=0; i<seg->get_number_paths(); i++) {
    ...
    for(size_t j=0; j<etqpart.size(); j++) {
      if(etqpart[j]==-1) {
	...
	cerr<<"Tag2: "<<tag2<<" "<<UtfConverter::toUtf8(tagger_data.getArrayTags()[tag2])<<"\n";
	exit(EXIT_FAILURE);
      }
